...
def create_upload_files(fileParts: List[bytes] = File(...), fileName: str = Form(...)):
    try:
        ...
        if (len(fileParts) != config['AMOUNT_OF_FILE_PARTS']):
            raise Exception("Invalid number of file parts where given!")

        processFile(path.join(config['IMAGES_PATH'], fileName), b''.join(fileParts), LocalJSONCommunicator(config['JSON_LOGS_PATH']))
